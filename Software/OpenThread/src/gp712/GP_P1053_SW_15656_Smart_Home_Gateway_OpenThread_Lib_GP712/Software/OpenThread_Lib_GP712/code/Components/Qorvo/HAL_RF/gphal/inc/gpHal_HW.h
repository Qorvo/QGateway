...
#define GP_LOG(fmt, ...)  GP_LOG_SYSTEM_PRINTF(fmt,  __VA_ARGS__)
#else
/* workaround for Holtek and others
 * to avoid varargs macro trouble with holtec we always define it
 * as GP_LOG_PRINTF, and not as an empty statement.
 * gpLog.h is responsible to make it an empty statement/function
 */
#define GP_LOG  GP_LOG_PRINTF
...
#endif //GP_DIVERSITY_DEVELOPMENT && defined(GP_DIVERSITY_LOG)
