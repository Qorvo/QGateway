 ...
 *
 * $Header: //depot/release/Embedded/Components/Qorvo/OS/v2.10.2.0/comps/gpLog/inc/gpLog.h#1 $
 * $Change: 187624 $
...
*/
#ifdef __GNUC__
GP_API void gpLog_Printf_Com(UInt8 componentID, Bool progmem, FLASH_STRING format_str, UInt8 length, ...) __attribute__((format(printf, 3, 5)));
#else
GP_API void gpLog_Printf_Com(UInt8 componentID, Bool progmem, FLASH_STRING format_str, UInt8 length, ...);
#endif
...
/**
*   @brief Enable or disable the default gpLog print to console on terminal based applications.
*
*   @param    enable    true or false to enable or disable the print
*/
GP_API void gpLog_EnableConsoleLog(Bool enable);

/**
*   @brief Retrieve the value of the 'gpLog print to console on terminal based applications' setting
*
*   @return   Bool      true if the print is enabled
*/
GP_API Bool gpLog_GetConsoleLog(void);

/**
*   @brief Set a file pointer to output the Logging to. File creation should be managed by the caller.
*
*   @param    pFile    file pointer to print to. If NULL print to file is disabled.
*/
GP_API void gpLog_SetFilePointer(void* pFile);
