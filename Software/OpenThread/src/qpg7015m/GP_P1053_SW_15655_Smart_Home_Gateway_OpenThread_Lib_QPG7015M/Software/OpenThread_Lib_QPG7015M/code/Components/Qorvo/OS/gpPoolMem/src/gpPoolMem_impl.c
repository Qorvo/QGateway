 ...
 *
 * $Header: //depot/release/Embedded/Components/Qorvo/OS/v2.10.2.0/comps/gpPoolMem/src/gpPoolMem_impl.c#1 $
 * $Change: 187624 $
...
#define POOLMEM_MALLOC(size,amount,nbytes,comp_id)    do { \
    ...
    { \
        ...
        { \
            GP_LOG_PRINTF("BA-%u[%u]-CId:%u-S:%u",8,(UInt16)sizeof(Chunk##size[i].Data),(UInt16)i,(UInt16)comp_id,(UInt32)nbytes);\
...
//Data pools
#ifdef GP_DIVERSITY_PD_USE_RAM_VARIANT
extern gpPoolMem_Chunkpd_t Chunkpd[];
#endif //GP_DIVERSITY_PD_USE_RAM_VARIANT
...
#ifdef GP_DIVERSITY_PD_USE_RAM_VARIANT
    POOLMEM_INIT(pd,GP_POOLMEM_PD_AMOUNT);
#endif //GP_DIVERSITY_PD_USE_RAM_VARIANT
...
void *PoolMem_Malloc (UInt8 comp_id, UInt32 nbytes, Bool try_)
{
...
#ifdef GP_DIVERSITY_PD_USE_RAM_VARIANT
        if (nbytes <= GP_POOLMEM_PD_SIZE) POOLMEM_MALLOC(pd,GP_POOLMEM_PD_AMOUNT,nbytes,comp_id);
#endif
    }
    ...
    GP_LOG_PRINTF("Chunk size too small: CId:%u - size:%u",4,(UInt16)comp_id,(UInt32)nbytes);
...
/* Check guards and free chunk, if guard error -> assert */
{
    // Release chunk
#ifdef GP_DIVERSITY_PD_USE_RAM_VARIANT
    POOLMEM_FREE(pd,GP_POOLMEM_PD_AMOUNT);
#endif //GP_DIVERSITY_PD_USE_RAM_VARIANT
...
#ifdef GP_DIVERSITY_PD_USE_RAM_VARIANT
    POOLMEM_FREE_BY_COMPID(pd,GP_POOLMEM_PD_AMOUNT, comp_id);
#endif //GP_DIVERSITY_PD_USE_RAM_VARIANT
...
#ifdef GP_DIVERSITY_PD_USE_RAM_VARIANT
    POOLMEM_IN_USE(pd,GP_POOLMEM_PD_AMOUNT,amountInUse);
#endif //GP_DIVERSITY_PD_USE_RAM_VARIANT
