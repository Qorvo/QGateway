...
MAIN_FUNCTION_RETURN_TYPE MAIN_FUNCTION_NAME(void)
{
    ...
    for (;;)
    {
        ...
        /* The scheduler task flag serves to notice events scheduled
         * asynchronously, we clear it now to avoid dropping out of sleep
         * immediately since CanGoToSleep will consider these events created up
         * until now. */
        hal_ClearWakeUpEvent();
...
/*
 * @brief Wake up a sleeping scheduler thread
 *
 * This function can used to wake up a sleeping scheduler thread.
 */
