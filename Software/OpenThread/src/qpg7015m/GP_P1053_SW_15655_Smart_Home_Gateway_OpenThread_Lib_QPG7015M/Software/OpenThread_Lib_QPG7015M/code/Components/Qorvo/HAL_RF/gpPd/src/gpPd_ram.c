 ...
 *
 * $Header: //depot/release/Embedded/Components/Qorvo/HAL_RF/v2.10.3.2/comps/gpPd/src/gpPd_ram.c#1 $
 * $Change: 198093 $
...
/** @brief Check a given buffer type is valid
 *
 *  @param type Pd buffer type to check
*/
...
static void Pd_DataIndication(UInt8 pbmHandle, gpPd_Offset_t pbmOffset, gpPd_Length_t pbmLength, gpPd_Loh_t *p_PdLoh, gpPd_BufferType_t type)
{
    ...
    if( p_PdLoh->handle != GP_PD_INVALID_HANDLE )
    {
        ...
        gpPd_Descriptors[p_PdLoh->handle]->attr.rx.txAckFc = gpHal_GetFrameControlFromTxAckAfterRx(pbmHandle);
        gpPd_Descriptors[p_PdLoh->handle]->attr.rx.txAckKeyId = gpHal_GetKeyIdFromTxAckAfterRx(pbmHandle);
        gpPd_Descriptors[p_PdLoh->handle]->attr.rx.txAckFrameCounter = gpHal_GetFrameCounterFromTxAckAfterRx(pbmHandle);
