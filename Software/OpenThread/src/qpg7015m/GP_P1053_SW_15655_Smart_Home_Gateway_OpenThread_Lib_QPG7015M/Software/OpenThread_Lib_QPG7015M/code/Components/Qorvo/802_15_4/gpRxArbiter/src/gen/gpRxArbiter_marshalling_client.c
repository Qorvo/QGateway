 ...
 *
 * $Header: //depot/release/Embedded/Components/Qorvo/802_15_4/v2.10.3.1/comps/gpRxArbiter/src/gen/gpRxArbiter_marshalling_client.c#1 $
 * $Change: 197210 $
...
{
    UInt32_api2buf_1(pDest, &(onTimeUs), pIndex);
    UInt32_api2buf_1(pDest, &(offTimeUs), pIndex);
    gpRxArbiter_StackId_t_api2buf_1(pDest, &(stackId), pIndex);
}
...
void gpRxArbiter_EnableRxWindows_Output_buf2par(gpRxArbiter_Result_t* result , UInt8 channel , UInt32 dutyCycleOnTime , UInt32 dutyCyclePeriod , UInt16 recurrenceAmount , UInt32 startTime , gpRxArbiter_StackId_t stackId , UInt8Buffer* pSource , UInt16* pIndex)
{
    gpRxArbiter_Result_t_buf2api(result, pSource, 1, pIndex);
    NOT_USED(channel);
    NOT_USED(dutyCycleOnTime);
    NOT_USED(dutyCyclePeriod);
    NOT_USED(recurrenceAmount);
    NOT_USED(startTime);
    NOT_USED(stackId);
}
