...
#ifdef GP_FREERTOS_DIVERSITY_NO_GPSCHED
#include "hal_SchedFreeRTOS.h"
#endif //GP_FREERTOS_DIVERSITY_NO_GPSCHED
#include "gpSched.h"
...
#ifdef GP_COM_DIVERSITY_PACKET_FILTERING
static Bool isFilterPatternInPacket(gpCom_FilterPattern_t* pPattern, UInt16 length, UInt8* pData);
...
void gpCom_HandleRxNonBlocking(void)
{
    ...
    if (!HAL_IS_MUTEX_ACQUIRED(Com_MultiThreadingMutex))
    {
...
void Com_cbPacketReceived(gpCom_Packet_t* pPacket)
{
    ...
    if(gpCom_FilterIsPacketToReject(queueUsagePer, pPacket))
    {
        Com_FreePacket(pPacket);
        return;
    }
...
Bool gpCom_RemoveFilter(UInt8 moduleId, UInt8 bufferUsageThresh, UInt8 patternsCnt, const gpCom_FilterPattern_t* patterns)
{
    UInt8 i, j;
    ...
    for(i = 0; i < GP_COM_FILTER_CNT; i++)
    {
        if(filters[i].enabled && filters[i].moduleId == moduleId 
            && filters[i].bufferUsageThresh == bufferUsageThresh 
            && patternsCnt == filters[i].patternsCnt 
            && isPatternEqual(patterns, filters[i].patterns, filters[i].patternsCnt))
        {
            break;
        }
    }
...
Bool gpCom_FilterIsPacketToReject(UInt8 queueUsagePer, gpCom_Packet_t* pPacket)
{
    return gpCom_FilterIsPacketDataToReject(queueUsagePer, pPacket->moduleID, pPacket->length, pPacket->packet, pPacket->commId);
}

Bool gpCom_FilterIsPacketDataToReject(UInt8 queueUsagePer, UInt8 moduleID, UInt16 length, UInt8* pData, gpCom_CommunicationId_t commId)
{
    UInt8 filterIdx;
    UInt8 patternIdx;
    ...
    for(filterIdx = 0; (filterIdx < GP_COM_FILTER_CNT) && !patternFound; filterIdx++)
    {
        ...
        if(!filters[filterIdx].enabled || (filters[filterIdx].moduleId != moduleID))
        {
            continue;
        }
        ...
        {
            patternFound = isFilterPatternInPacket(&filters[filterIdx].patterns[patternIdx], length, pData);
...
static Bool isFilterPatternInPacket(gpCom_FilterPattern_t* pPattern, UInt16 length, UInt8* pData)
{
    ...
    if((pPattern->offset + pPattern->len) > length)
    {
        return false;
    }

    // Compare pattern data with the packet data on given offset:
    if(!memcmp(&pData[pPattern->offset], pPattern->data, pPattern->len))
    {
        return true;
    }
