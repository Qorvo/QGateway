...
void gpMacDispatcher_ScheduleTimedTx_Input_par2buf(UInt8Buffer* pDest , gpPd_Handle_t pdHandle , gpMacCore_TxTimingOptions_t timingOptions , gpMacDispatcher_StackId_t stackId , UInt16* pIndex)
{
    ...
    pDest[(*pIndex)++] = pdHandle;
#endif
    gpMacCore_TxTimingOptions_t_api2buf(pDest, &(timingOptions), 1, pIndex);
    gpMacDispatcher_StackId_t_api2buf_1(pDest, &(stackId), pIndex);
}
...
{
    Bool_api2buf_1(pDest, &(rawModeEnabled), pIndex);
    gpMacDispatcher_StackId_t_api2buf_1(pDest, &(stackId), pIndex);
}
...
{
    Bool_buf2api(rawModeEnabled, pSource, 1, pIndex);
    NOT_USED(stackId);
}
...
{
    Bool_api2buf_1(pDest, &(enableEnhancedFramePending), pIndex);
    gpMacDispatcher_StackId_t_api2buf_1(pDest, &(stackId), pIndex);
}
...
{
    UInt32_api2buf_1(pDest, &(dutyCycleOnTime), pIndex);
    UInt32_api2buf_1(pDest, &(dutyCyclePeriod), pIndex);
    UInt16_api2buf_1(pDest, &(recurrenceAmount), pIndex);
    UInt32_api2buf_1(pDest, &(startTime), pIndex);
    gpMacDispatcher_StackId_t_api2buf_1(pDest, &(stackId), pIndex);
}
...
{
    UInt16_api2buf_1(pDest, &(dutyCyclePeriod), pIndex);
    gpMacDispatcher_StackId_t_api2buf_1(pDest, &(stackId), pIndex);
}
...
{
    UInt8_api2buf_1(pDest, &(linkMetrics), pIndex);
    pDest[(*pIndex)++] = (NULL == pExtendedAddress);
    if (NULL != pExtendedAddress)
    {
        MACAddress_t_api2buf_1(pDest, pExtendedAddress, pIndex);
...
{
    gpMacCore_Result_t_buf2api(result, pSource, 1, pIndex);
    NOT_USED(linkMetrics);
    NOT_USED(pExtendedAddress);
...
{
    UInt8_api2buf_1(pDest, &(minBERetransmit), pIndex);
    gpMacCore_StackId_t_api2buf_1(pDest, &(stackId), pIndex);
}
...
{
    UInt8_buf2api(minBERetransmit, pSource, 1, pIndex);
    NOT_USED(stackId);
}
...
{
    UInt8_api2buf_1(pDest, &(maxBERetransmit), pIndex);
    gpMacCore_StackId_t_api2buf_1(pDest, &(stackId), pIndex);
}
...
{
    UInt8_buf2api(maxBERetransmit, pSource, 1, pIndex);
    NOT_USED(stackId);
}
...
gpMarshall_AckStatus_t MacDispatcher_cbDataConfirm_Input_buf2api(MacDispatcher_cbDataConfirm_Input_marshall_struct_t* pDest , UInt8Buffer* pSource , UInt16* pIndex)
{
    ...
    /* server had pdHandle stored */
#ifdef GP_COMP_UNIT_TEST
    pDest->data.pdHandle = gpPd_GetRealPd(pSource[(*pIndex)++]);
#else
    pDest->data.pdHandle = pSource[(*pIndex)++];
#endif
    {
    ...
    {
        Bool enhancedAck;
        Bool_buf2api(&enhancedAck , pSource, 1, pIndex );
        gpPd_SetRxEnhancedAckFromTxPbm(pDest->data.pdHandle , enhancedAck);
    }
...
gpMarshall_AckStatus_t MacDispatcher_cbDataIndication_Input_buf2api(MacDispatcher_cbDataIndication_Input_marshall_struct_t* pDest , UInt8Buffer* pSource , UInt16* pIndex)
{
    ...
    {
        UInt16 framecontrol;
        UInt16_buf2api(&framecontrol , pSource, 1, pIndex );
        gpPd_SetFrameControlFromTxAckAfterRx(pDest->data.pdLoh.handle , framecontrol);
    }

    {
        UInt32 framecounter;
        UInt32_buf2api(&framecounter , pSource, 1, pIndex );
        gpPd_SetFrameCounterFromTxAckAfterRx(pDest->data.pdLoh.handle , framecounter);
    }
    {
        UInt8 keyid;
        UInt8_buf2api(&keyid , pSource, 1, pIndex );
        gpPd_SetKeyIdFromTxAckAfterRx(pDest->data.pdLoh.handle , keyid);
    }
    gpMacDispatcher_StackId_t_buf2api(&(pDest->data.stackId), pSource, 1, pIndex);
    return gpMarshall_AckStatusSuccess;
}
