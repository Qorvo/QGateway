...
if [ "${LIB_TYPE}" = "rcp" ] ; then
    if [ "${QORVO_PLATFORM}" = "gp712" ] ; then
        "${OTDIR}"/script/build "${QORVO_PLATFORM}" -DOT_APP_RCP=1 -DOT_RCP=1 -DOT_QORVO_SOCKET=0 -DOT_COMMISSIONER=1 -DOT_JOINER=1 -DOT_DHCP6_CLIENT=1 -DOT_DHCP6_SERVER=1 -DOT_REFERENCE_DEVICE=ON -DOT_THREAD_VERSION=1.1
    else
        "${OTDIR}"/script/build "${QORVO_PLATFORM}" -DOT_APP_RCP=1 -DOT_RCP=1 -DOT_QORVO_SOCKET=0 -DOT_COMMISSIONER=1 -DOT_JOINER=1 -DOT_DHCP6_CLIENT=1 -DOT_DHCP6_SERVER=1 -DOT_REFERENCE_DEVICE=ON
    fi
elif [ "${LIB_TYPE}" = "ftd" ] ; then
    if [ "${QORVO_PLATFORM}" = "gp712" ] ; then
        "${OTDIR}"/script/build "${QORVO_PLATFORM}" -DOT_APP_CLI=1 -DOT_FTD=1 -DOT_QORVO_SOCKET=0 -DOT_COMMISSIONER=1 -DOT_JOINER=1 -DOT_DHCP6_CLIENT=1 -DOT_DHCP6_SERVER=1 -DOT_REFERENCE_DEVICE=ON -DOT_THREAD_VERSION=1.1
    else
        "${OTDIR}"/script/build "${QORVO_PLATFORM}" -DOT_APP_CLI=1 -DOT_FTD=1 -DOT_QORVO_SOCKET=0 -DOT_COMMISSIONER=1 -DOT_JOINER=1 -DOT_DHCP6_CLIENT=1 -DOT_DHCP6_SERVER=1 -DOT_REFERENCE_DEVICE=ON
    fi 
fi
}
...
if [ "${BUILD_GP712}" = 1 ] ; then
    platform_build "gp712" "${GP712_LIB_PACKAGE}" "${GP712_LIB_SOURCE}" "${GP712_LIB_TARGET}" "ftd" "-DOT_THREAD_VERSION=1.1"
    platform_build "gp712" "${GP712_LIB_PACKAGE}" "${GP712_LIB_RCP_SOURCE}" "${GP712_LIB_RCP_TARGET}" "rcp" "-DOT_THREAD_VERSION=1.1"
fi
